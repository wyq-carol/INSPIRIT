...
void _starpu_init_all_sched_ctxs(struct _starpu_machine_config *config)
{
  ...
  for (i = 0; i <= STARPU_NMAX_SCHED_CTXS; i++)
  {
    ...
    // __gzh__
    config->sched_ctxs[i].nready_k_list = (config->conf).nready_k_list;
    config->sched_ctxs[i].nready_k_list_size = (config->conf).nready_k_list_size;
    config->sched_ctxs[i].nready_lb_list = (config->conf).nready_lb_list;
    config->sched_ctxs[i].nready_lb_list_size = (config->conf).nready_lb_list_size;
...
double *starpu_sched_ctx_get_nready_k_list(unsigned sched_ctx_id)
{
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
  return sched_ctx->nready_k_list;
}
...
int starpu_sched_ctx_get_nready_k_list_size(unsigned sched_ctx_id)
{
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
  return sched_ctx->nready_k_list_size;
}
...
int *starpu_sched_ctx_get_nready_lb_list(unsigned sched_ctx_id)
{
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
  return sched_ctx->nready_lb_list;
}
...
int starpu_sched_ctx_get_nready_lb_list_size(unsigned sched_ctx_id)
{
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
  return sched_ctx->nready_lb_list_size;
}
