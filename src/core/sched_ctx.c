...
static starpu_pthread_mutex_t sched_ctx_manag =
    STARPU_PTHREAD_MUTEX_INITIALIZER;
static starpu_pthread_mutex_t finished_submit_mutex =
    STARPU_PTHREAD_MUTEX_INITIALIZER;
...
static unsigned _starpu_get_first_free_sched_ctx(
    struct _starpu_machine_config *config);

static void _starpu_sched_ctx_put_new_master(unsigned sched_ctx_id);
static void _starpu_sched_ctx_block_workers_in_parallel(unsigned sched_ctx_id,
                                                        unsigned all);
static void _starpu_sched_ctx_unblock_workers_in_parallel(unsigned sched_ctx_id,
                                                          unsigned all);
static void _starpu_sched_ctx_update_parallel_workers_with(
    unsigned sched_ctx_id);
static void _starpu_sched_ctx_update_parallel_workers_without(
    unsigned sched_ctx_id);

static void set_priority_on_notified_workers(int *workers, int nworkers,
                                             unsigned sched_ctx_id,
                                             unsigned priority);
static void set_priority_hierarchically_on_notified_workers(
    int *workers_to_add, unsigned nworkers_to_add, unsigned sched_ctx,
    unsigned priority);
static void fetch_tasks_from_empty_ctx_list(
    struct _starpu_sched_ctx *sched_ctx);
static void add_notified_workers(int *workers_to_add, int nworkers_to_add,
                                 unsigned sched_ctx_id);
 ...
 * done.
 */
static void notify_workers_about_changing_ctx_pending(
    const unsigned nworkers, const int *const workerids)
{
  STARPU_ASSERT(!_starpu_worker_sched_op_pending());
  const int cur_workerid = _starpu_worker_get_id();
  unsigned i;
  for (i = 0; i < nworkers; i++) {
    /* check that workerids[] is sorted to prevent multi-lock acquisition
     * deadlocks */
    STARPU_ASSERT(i == 0 || (workerids[i] > workerids[i - 1]));
    if (starpu_worker_is_combined_worker(workerids[i])) continue;
    if (workerids[i] == cur_workerid) continue;
 ...
 * workerids must be sorted by ascending id
 *
 * Once this function returns, the workers may proceed with scheduling
 * operations again.
 */
static void notify_workers_about_changing_ctx_done(const unsigned nworkers,
                                                   const int *const workerids)
{
  STARPU_ASSERT(!_starpu_worker_sched_op_pending());
  const int cur_workerid = _starpu_worker_get_id();
  unsigned i;
...
static void _starpu_worker_gets_into_ctx(unsigned sched_ctx_id,
                                         struct _starpu_worker *worker)
{
  unsigned ret_sched_ctx =
      _starpu_sched_ctx_elt_exists(worker->sched_ctx_list, sched_ctx_id);
  /* the worker was planning to go away in another ctx but finally he changed
  ...
  if (worker->tmp_sched_ctx == (int)sched_ctx_id) worker->tmp_sched_ctx = -1;
  return;
}
                                    ...
                                    struct _starpu_worker *worker)
{
  ...
  if (ret_sched_ctx) {
    /* don't remove scheduling data here, there might be tasks running and when
       post_exec executes scheduling data is not there any more, do it when
       deleting context, then we really won't need it anymore */
    /* struct _starpu_sched_ctx *sched_ctx =
     * _starpu_get_sched_ctx_struct(sched_ctx_id); */
    /* if(sched_ctx && sched_ctx->sched_policy &&
     * sched_ctx->sched_policy->remove_workers) */
    /* { */
    /* 	_STARPU_SCHED_BEGIN; */
    /* 	sched_ctx->sched_policy->remove_workers(sched_ctx_id, &worker->workerid,
     * 1); */
    /* 	_STARPU_SCHED_END; */
    /* } */
...
static void _starpu_update_notified_workers_with_ctx(int *workerids,
                                                     int nworkers,
                                                     int sched_ctx_id)
{
  int i;
...
static void _starpu_update_notified_workers_without_ctx(int *workerids,
                                                        int nworkers,
                                                        int sched_ctx_id,
                                                        unsigned now)
{
...
static void _do_add_notified_workers(struct _starpu_sched_ctx *sched_ctx,
                                     int *workerids, int nworkers)
{
  int ndevices = 0;
  ...
  if (ndevices > 0) {
    if (sched_ctx->perf_arch.devices == NULL) {
      _STARPU_MALLOC(sched_ctx->perf_arch.devices,
                     ndevices * sizeof(struct starpu_perfmodel_device));
    } else {
      ...
      for (dev1 = 0; dev1 < ndevices; dev1++) {
        for (dev2 = 0; dev2 < sched_ctx->perf_arch.ndevices; dev2++) {
          if (sched_ctx->perf_arch.devices[dev2].type == devices[dev1].type &&
              sched_ctx->perf_arch.devices[dev2].devid == devices[dev1].devid)
            found = 1;
        }
      ...
      _STARPU_REALLOC(sched_ctx->perf_arch.devices,
                      nsize * sizeof(struct starpu_perfmodel_device));
    }
    ...
    for (dev1 = 0; dev1 < ndevices; dev1++) {
      ...
      if (!found) {
        sched_ctx->perf_arch.devices[sched_ctx->perf_arch.ndevices].type =
            devices[dev1].type;
        sched_ctx->perf_arch.devices[sched_ctx->perf_arch.ndevices].devid =
            devices[dev1].devid;
        if (sched_ctx->stream_worker != -1)
          sched_ctx->perf_arch.devices[sched_ctx->perf_arch.ndevices].ncores =
              sched_ctx->nsms;
        else
          sched_ctx->perf_arch.devices[sched_ctx->perf_arch.ndevices].ncores =
              devices[dev1].ncores;
...
static void _starpu_add_workers_to_new_sched_ctx(
    struct _starpu_sched_ctx *sched_ctx, int *workerids, int nworkers)
{
  struct starpu_worker_collection *workers = sched_ctx->workers;
  struct _starpu_machine_config *config = _starpu_get_machine_config();
  if (nworkers == -1) nworkers = config->topology.nworkers;
  if (!nworkers) return;
  int _workerids[nworkers];
  int i;
  if (workerids == NULL) {
    for (i = 0; i < nworkers; i++) _workerids[i] = i;
...
static void _starpu_remove_workers_from_sched_ctx(
    struct _starpu_sched_ctx *sched_ctx, int *workerids, int nworkers,
    int *removed_workers, int *n_removed_workers)
{
  ...
  for (i = 0; i < nworkers; i++) {
    if (workers->nworkers > 0) {
      if (_starpu_worker_belongs_to_a_sched_ctx(workerids[i], sched_ctx->id)) {
        ...
        if (worker >= 0) removed_workers[(*n_removed_workers)++] = worker;
      }
    }
  }
  ...
  if (workers->init_iterator) workers->init_iterator(workers, &it);
...
static void _starpu_sched_ctx_free_scheduling_data(
    struct _starpu_sched_ctx *sched_ctx)
{
  if (sched_ctx->sched_policy && sched_ctx->sched_policy->remove_workers) {
    ...
    unsigned nworkers_ctx =
        starpu_sched_ctx_get_workers_list(sched_ctx->id, &workerids);

    if (nworkers_ctx > 0) {
      _STARPU_SCHED_BEGIN;
      sched_ctx->sched_policy->remove_workers(sched_ctx->id, workerids,
                                              nworkers_ctx);
      _STARPU_SCHED_END;
    }
...
static void _starpu_sched_ctx_create_hwloc_tree(
    struct _starpu_sched_ctx *sched_ctx)
{
  ...
  while (workers->has_next(workers, &it)) {
    ...
    if (!starpu_worker_is_combined_worker(workerid)) {
      ...
      hwloc_bitmap_or(sched_ctx->hwloc_workers_set,
                      sched_ctx->hwloc_workers_set, worker->hwloc_cpu_set);
    }
  }
  return;
}
...
struct _starpu_sched_ctx *_starpu_create_sched_ctx(
    struct starpu_sched_policy *policy, int *workerids, int nworkers_ctx,
    unsigned is_initial_sched, const char *sched_ctx_name, int min_prio_set,
    int min_prio, int max_prio_set, int max_prio, unsigned awake_workers,
    void (*sched_policy_callback)(unsigned), void *user_data, int nsub_ctxs,
    int *sub_ctxs, int nsms)
{
  struct _starpu_machine_config *config = _starpu_get_machine_config();
  ...
  for (i = 0; i < (int)(sizeof(sched_ctx->iterations) /
                        sizeof(sched_ctx->iterations[0]));
       i++)
  ...
  if (nsms > 0) {
    ...
    STARPU_ASSERT_MSG(occupied_sms <= STARPU_NMAXSMS,
                      "STARPU:requested more sms than available");
  ...
  /*init the strategy structs and the worker_collection of the ressources of the
   * context */
  if (policy) {
  ...
  /*add sub_ctxs before add workers, in order to be able to associate them if
   * necessary */
  if (nsub_ctxs != 0) {
    for (i = 0; i < nsub_ctxs; i++) sched_ctx->sub_ctxs[i] = sub_ctxs[i];
   ...
   * Make sure the sched_ctx struct and the policy struct initialization are
...
void starpu_sched_ctx_get_sms_interval(int stream_workerid, int *start,
                                       int *end)
{
...
int starpu_sched_ctx_get_sub_ctxs(unsigned sched_ctx, int *ctxs)
{
  struct _starpu_sched_ctx *sc = _starpu_get_sched_ctx_struct(sched_ctx);
  int i;
  for (i = 0; i < sc->nsub_ctxs; i++) ctxs[i] = sc->sub_ctxs[i];
  return sc->nsub_ctxs;
}
...
unsigned starpu_sched_ctx_create(int *workerids, int nworkers,
                                 const char *sched_ctx_name, ...)
{
  ...
  struct _starpu_sched_ctx *sched_ctx;
  sched_ctx = _starpu_create_sched_ctx(
      sched_policy, workerids, nworkers, 0, sched_ctx_name, min_prio_set,
      min_prio, max_prio_set, max_prio, awake_workers, init_sched, user_data,
      nsub_ctxs, sub_ctxs, nsms);
  sched_ctx->hierarchy_level = hierarchy_level;
  sched_ctx->nesting_sched_ctx = nesting_sched_ctx;

  int *added_workerids;
  unsigned nw_ctx =
      starpu_sched_ctx_get_workers_list(sched_ctx->id, &added_workerids);
  ...
  _starpu_update_notified_workers_with_ctx(added_workerids, nw_ctx,
                                           sched_ctx->id);
...
int fstarpu_sched_ctx_create(int *workerids, int nworkers,
                             const char *sched_ctx_name, void **arglist)
{
  STARPU_PTHREAD_MUTEX_LOCK(&sched_ctx_manag);
  int arg_i = 0;
  int min_prio_set = 0;
...
void starpu_sched_ctx_register_close_callback(
    unsigned sched_ctx_id,
    void (*close_callback)(unsigned sched_ctx_id, void *args), void *args)
{
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
...
void starpu_sched_ctx_delete(unsigned sched_ctx_id)
{
...
#ifdef STARPU_USE_SC_HYPERVISOR
  if (sched_ctx_id != 0 && sched_ctx_id != STARPU_NMAX_SCHED_CTXS &&
      sched_ctx->perf_counters != NULL) {
  ...
  struct _starpu_sched_ctx *inheritor_sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx->inheritor);
  ...
  memcpy(backup_workerids, workerids,
         nworkers_ctx * sizeof(backup_workerids[0]));
  ...
  if (nworkers_ctx > 0 && inheritor_sched_ctx &&
      inheritor_sched_ctx->id != STARPU_NMAX_SCHED_CTXS &&
      !(nworkers_ctx == nworkers &&
        nworkers_ctx == inheritor_sched_ctx->workers->nworkers)) {
  ...
  if (!wait_status) {
    ...
    /*if btw the mutex release & the mutex lock the context has changed take
      care to free all scheduling data before deleting the context */
    ...
    _starpu_update_notified_workers_without_ctx(workerids, nworkers_ctx,
                                                sched_ctx_id, 1);
  ...
  /* workerids is malloc-ed in starpu_sched_ctx_get_workers_list, don't forget
     to free it when you don't use it anymore */
  free(workerids);
  STARPU_PTHREAD_MUTEX_UNLOCK(&sched_ctx_manag);
}

/* called after the workers are terminated so we don't have anything else to do
 * but free the memory*/
void starpu_sched_ctx_print_nready_time_list(unsigned sched_ctx_id) {
	struct _starpu_sched_ctx *sched_ctx = _starpu_get_sched_ctx_struct(sched_ctx_id);
	int plist = sched_ctx->p_list;
	int *nready_list = sched_ctx->nready_list;
	double *time_list = sched_ctx->time_list;
	printf("nready_list = [");
	for (int i = 0; i < plist - 1; i++)
	{
		printf("%d,", nready_list[i]);
	}
	printf("%d]\n", nready_list[plist - 1]);
	...
	for (int i = 0; i < plist - 1; i++)
	{
		printf("%lf,", (time_list[i] - time_list[0]) * 1000);
	}
	printf("%lf]\n", (time_list[plist - 1] - time_list[0]) * 1000);
}
void _starpu_delete_all_sched_ctxs()
{
  STARPU_PTHREAD_MUTEX_LOCK(&sched_ctx_manag);

  starpu_sched_ctx_print_nready_time_list(0);
...
static void _starpu_check_workers(int *workerids, int nworkers)
{
  ...
  for (i = 0; i < nworkers; i++) {
    ...
    STARPU_ASSERT_MSG(
        workerids[i] >= 0 && workerids[i] <= nworkers_conf,
        "requested to add workerid = %d, but that is beyond the range 0 to %d",
        workerids[i], nworkers_conf);
  }
}
...
static void fetch_tasks_from_empty_ctx_list(struct _starpu_sched_ctx *sched_ctx)
{
  ...
  while (!starpu_task_list_empty(&list)) {
    ...
    if (old_task == &stop_submission_task) break;
...
unsigned _starpu_can_push_task(struct _starpu_sched_ctx *sched_ctx,
                               struct starpu_task *task)
{
  if (sched_ctx->sched_policy && sched_ctx->sched_policy->simulate_push_task) {
    ...
    if (expected_len > (window_size + 0.2 * window_size)) return 0;
  }
  return 1;
}
...
{
  if (starpu_task_list_empty(&sched_ctx->waiting_tasks)) return;
  struct starpu_task *old_task =
      starpu_task_list_back(&sched_ctx->waiting_tasks);
...
void _starpu_push_task_to_waiting_list(struct _starpu_sched_ctx *sched_ctx,
                                       struct starpu_task *task)
{
    ...
    unsigned priority)
{
  if (starpu_sched_ctx_get_hierarchy_level(sched_ctx) > 0) {
    ...
    set_priority_on_notified_workers(workers_to_add, nworkers_to_add, father,
                                     priority);
    set_priority_hierarchically_on_notified_workers(
        workers_to_add, nworkers_to_add, father, priority);
  }
  return;
}

static void add_notified_workers(int *workerids, int nworkers,
                                 unsigned sched_ctx_id)
{
  if (!nworkers) return;
  struct _starpu_sched_ctx *sched_ctx =
  ...
  if (sched_ctx->id == STARPU_NMAX_SCHED_CTXS) return;
  ...
  if (n_added_workers > 0) {
    if (sched_ctx->sched_policy && sched_ctx->sched_policy->add_workers) {
      _STARPU_SCHED_BEGIN;
      sched_ctx->sched_policy->add_workers(sched_ctx->id, added_workers,
                                           n_added_workers);
      _STARPU_SCHED_END;
    }
    _starpu_update_notified_workers_with_ctx(added_workers, n_added_workers,
                                             sched_ctx->id);
  }
  set_priority_on_notified_workers(workerids, nworkers, sched_ctx_id, 1);
  set_priority_hierarchically_on_notified_workers(workerids, nworkers,
                                                  sched_ctx_id, 0);
  fetch_tasks_from_empty_ctx_list(sched_ctx);
}

/* Queue a new ctx change operation in the list of deferred ctx changes of the
 * current worker.
 *
...
static void _defer_ctx_change(int sched_ctx_id, enum _starpu_ctx_change_op op,
                              int nworkers_to_notify, int *workerids_to_notify,
                              int nworkers_to_change, int *workerids_to_change)
{
  STARPU_ASSERT(_starpu_worker_sched_op_pending());
  if (nworkers_to_change == 0) return;
  ...
  _STARPU_MALLOC(chg->workerids_to_change,
                 nworkers_to_change * sizeof(chg->workerids_to_change[0]));
  memcpy(chg->workerids_to_change, workerids_to_change,
         nworkers_to_change * sizeof(chg->workerids_to_change[0]));
  if (nworkers_to_notify != 0) {
    STARPU_ASSERT(workerids_to_notify != NULL);
    chg->nworkers_to_notify = nworkers_to_notify;
    _STARPU_MALLOC(chg->workerids_to_notify,
                   nworkers_to_notify * sizeof(chg->workerids_to_notify[0]));
    memcpy(chg->workerids_to_notify, workerids_to_notify,
           nworkers_to_notify * sizeof(chg->workerids_to_notify[0]));
  } else {
    STARPU_ASSERT(workerids_to_notify == NULL);
...
void starpu_sched_ctx_add_workers(int *workers_to_add, unsigned nworkers_to_add,
                                  unsigned sched_ctx_id)
{
  ...
  unsigned ctx_nworkers =
      starpu_sched_ctx_get_workers_list_raw(sched_ctx_id, &ctx_workerids);
  _starpu_sched_ctx_unlock_read(sched_ctx_id);
  int cumulated_workerids[ctx_nworkers + nworkers_to_add];
  memcpy(cumulated_workerids, ctx_workerids,
         ctx_nworkers * sizeof(cumulated_workerids[0]));
  ...
  if (_starpu_worker_sched_op_pending()) {
    _defer_ctx_change(sched_ctx_id, ctx_change_add, cumulated_nworkers,
                      cumulated_workerids, nworkers_to_add, workers_to_add);
  } else {
    sort_workerid_array(cumulated_nworkers, cumulated_workerids);
    notify_workers_about_changing_ctx_pending(cumulated_nworkers,
                                              cumulated_workerids);
    _starpu_sched_ctx_lock_write(sched_ctx_id);
    add_notified_workers(workers_to_add, nworkers_to_add, sched_ctx_id);
    notify_workers_about_changing_ctx_done(cumulated_nworkers,
                                           cumulated_workerids);
    _starpu_sched_ctx_unlock_write(sched_ctx_id);
  }
}

void starpu_sched_ctx_add_combined_workers(int *combined_workers_to_add,
                                           unsigned n_combined_workers_to_add,
                                           unsigned sched_ctx_id)
{
  _starpu_sched_ctx_lock_write(sched_ctx_id);
  add_notified_workers(combined_workers_to_add, n_combined_workers_to_add,
                       sched_ctx_id);
  _starpu_sched_ctx_unlock_write(sched_ctx_id);
}

static void remove_notified_workers(int *workerids, int nworkers,
                                    unsigned sched_ctx_id)
{
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
  ...
  _starpu_remove_workers_from_sched_ctx(sched_ctx, workerids, nworkers,
                                        removed_workers, &n_removed_workers);

  if (n_removed_workers > 0) {
    _starpu_update_notified_workers_without_ctx(
        removed_workers, n_removed_workers, sched_ctx_id, 0);
    set_priority_on_notified_workers(removed_workers, n_removed_workers,
                                     sched_ctx_id, 1);
  }
}

void starpu_sched_ctx_remove_workers(int *workers_to_remove,
                                     ...
                                     unsigned sched_ctx_id)
{
  ...
  if (sched_ctx->id != STARPU_NMAX_SCHED_CTXS) {
    if (_starpu_worker_sched_op_pending()) {
      _defer_ctx_change(sched_ctx_id, ctx_change_remove, cumulated_nworkers,
                        cumulated_workerids, nworkers_to_remove,
                        workers_to_remove);
    } else {
      sort_workerid_array(cumulated_nworkers, cumulated_workerids);
      notify_workers_about_changing_ctx_pending(cumulated_nworkers,
                                                cumulated_workerids);
      _starpu_sched_ctx_lock_write(sched_ctx_id);
      remove_notified_workers(workers_to_remove, nworkers_to_remove,
                              sched_ctx_id);
      notify_workers_about_changing_ctx_done(cumulated_nworkers,
                                             cumulated_workerids);
      _starpu_sched_ctx_unlock_write(sched_ctx_id);
...
void _starpu_init_all_sched_ctxs(struct _starpu_machine_config *config)
{
  ...
  for (i = 0; i <= STARPU_NMAX_SCHED_CTXS; i++) {
    ...
    config->sched_ctxs[i].auto_opt = (config->conf).auto_opt;
	config->sched_ctxs[i].p_list = 0;
 	config->sched_ctxs[i].time_list = (double *)malloc(50000 * 2 * sizeof(double));
  	config->sched_ctxs[i].nready_list = (int *)malloc(50000 * 2 * sizeof(int));
  }
  return;
}
    ...
    struct _starpu_machine_config *config)
{
  unsigned i;
  for (i = 0; i < STARPU_NMAX_SCHED_CTXS; i++)
    if (config->sched_ctxs[i].id == STARPU_NMAX_SCHED_CTXS) return i;
...
int _starpu_wait_for_all_tasks_of_sched_ctx(unsigned sched_ctx_id)
{
  ...
  STARPU_ASSERT_MSG(
      ...
      "starpu_task_wait_for_all must not be called from a task or callback");
                                                    ...
                                                    unsigned n)
{
  ...
  STARPU_ASSERT_MSG(_starpu_worker_may_perform_blocking_calls(),
                    "starpu_task_wait_for_n_submitted_tasks must not be called "
                    "from a task or callback");

  return _starpu_barrier_counter_wait_until_counter_reaches_down_to_n(
      &sched_ctx->tasks_barrier, n);
}
...
void _starpu_decrement_nsubmitted_tasks_of_sched_ctx(unsigned sched_ctx_id)
{
  ...
  if (!config->watchdog_ok) config->watchdog_ok = 1;
#endif

  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
  int reached =
      _starpu_barrier_counter_get_reached_start(&sched_ctx->tasks_barrier);
  int finished = reached == 1;

  /* when finished decrementing the tasks if the user signaled he will not
     submit tasks anymore we can move all its workers to the inheritor context
   */
  if (finished && sched_ctx->inheritor != STARPU_NMAX_SCHED_CTXS) {
    ...
    if (sched_ctx->finished_submit) {
      ...
      if (sched_ctx->id != STARPU_NMAX_SCHED_CTXS) {
        ...
        if (nworkers > 0) {
          starpu_sched_ctx_add_workers(workerids, nworkers,
                                       sched_ctx->inheritor);
          free(workerids);
        }
      }
      _starpu_barrier_counter_decrement_until_empty_counter(
          &sched_ctx->tasks_barrier, 0.0);
      return;
    }
    STARPU_PTHREAD_MUTEX_UNLOCK(&finished_submit_mutex);
  }
...
unsigned _starpu_increment_nready_tasks_of_sched_ctx(unsigned sched_ctx_id,
                                                     double ready_flops,
                                                     struct starpu_task *task)
{
  unsigned ret = 1;
  ...
  _starpu_barrier_counter_increment(&sched_ctx->ready_tasks_barrier,
                                    ready_flops);
...
void _starpu_decrement_nready_tasks_of_sched_ctx_locked(unsigned sched_ctx_id,
                                                        double ready_flops)
{
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
  _starpu_barrier_counter_decrement_until_empty_counter(
      &sched_ctx->ready_tasks_barrier, ready_flops);
}

void _starpu_decrement_nready_tasks_of_sched_ctx(unsigned sched_ctx_id,
                                                 double ready_flops)
{
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
...
int starpu_sched_ctx_get_nready_tasks(unsigned sched_ctx_id)
{
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
  return _starpu_barrier_counter_get_reached_start(
      &sched_ctx->ready_tasks_barrier);
}
...
double starpu_sched_ctx_get_nready_flops(unsigned sched_ctx_id)
{
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
  return _starpu_barrier_counter_get_reached_flops(
      &sched_ctx->ready_tasks_barrier);
}
...
int _starpu_wait_for_no_ready_of_sched_ctx(unsigned sched_ctx_id)
{
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
  _starpu_barrier_counter_wait_for_empty_counter(
      &sched_ctx->ready_tasks_barrier);
  return 0;
}
...
{
  if (sched_ctx)
    STARPU_PTHREAD_SETSPECIFIC(sched_ctx_key,
                               (void *)(uintptr_t)(*sched_ctx + 1));
  else
...
void starpu_sched_ctx_update_start_resizing_sample(unsigned sched_ctx_id,
                                                   double start_sample)
{
...
int starpu_sched_ctx_get_auto_opt(unsigned sched_ctx_id)
{
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
  return sched_ctx->auto_opt;
}

struct starpu_sched_policy *starpu_sched_ctx_get_sched_policy(
    unsigned sched_ctx_id)
{
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
...
struct starpu_worker_collection *starpu_sched_ctx_create_worker_collection(
    unsigned sched_ctx_id,
    enum starpu_worker_collection_type worker_collection_type)
{
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
  ...
  case STARPU_WORKER_TREE:
    ...
    sched_ctx->workers->init_iterator_for_parallel_tasks =
        starpu_worker_tree.init_iterator_for_parallel_tasks;
  ...
  default:
    ...
    sched_ctx->workers->init_iterator_for_parallel_tasks =
        starpu_worker_list.init_iterator_for_parallel_tasks;
...
unsigned starpu_sched_ctx_get_workers_list_raw(unsigned sched_ctx_id,
                                               int **workerids)
{
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
...
int _starpu_get_workers_of_sched_ctx(unsigned sched_ctx_id, int *pus,
                                     enum starpu_worker_archtype arch)
{
  ...
  while (workers->has_next(workers, &it)) {
    ...
    if (curr_arch == arch || arch == STARPU_ANY_WORKER) pus[npus++] = worker;
  }
                                              ...
                                              unsigned sched_ctx_id2)
{
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
  struct _starpu_sched_ctx *sched_ctx2 =
      _starpu_get_sched_ctx_struct(sched_ctx_id2);
  ...
  while (workers->has_next(workers, &it1)) {
    ...
    while (workers2->has_next(workers2, &it2)) {
      ...
      if (worker == worker2) shared_workers++;
    }
  }
...
unsigned starpu_sched_ctx_contains_worker(int workerid, unsigned sched_ctx_id)
{
  ...
  if (workers) {
    ...
    for (i = 0; i < workers->nworkers; i++)
      if (workerid == workers->workerids[i]) return 1;
  }
  return 0;
}

unsigned starpu_sched_ctx_contains_type_of_worker(
    enum starpu_worker_archtype arch, unsigned sched_ctx_id)
{
                                               ...
                                               unsigned sched_ctx_id)
{
  ...
  for (i = 0; i < STARPU_NMAX_SCHED_CTXS; i++) {
    ...
    if (sched_ctx && sched_ctx->id != STARPU_NMAX_SCHED_CTXS &&
        sched_ctx->id != sched_ctx_id)
      if (starpu_sched_ctx_contains_worker(workerid, sched_ctx->id)) return 1;
  }
  return 0;
}
...
void _starpu_sched_ctx_post_exec_task_cb(int workerid, struct starpu_task *task,
                                         size_t data_size2, uint32_t footprint)
{
  ...
  if (sched_ctx != NULL &&
    ...
    if (_starpu_sched_ctx_allow_hypervisor(sched_ctx->id) ||
        task->hypervisor_tag > 0) {
      _STARPU_TRACE_HYPERVISOR_BEGIN();
      sched_ctx->perf_counters->notify_post_exec_task(
          task, data_size[task->sched_ctx][workerid], footprint,
...
void starpu_sched_ctx_call_pushed_task_cb(int workerid, unsigned sched_ctx_id)
{
  ...
  if (sched_ctx != NULL &&
      sched_ctx_id != _starpu_get_initial_sched_ctx()->id &&
      sched_ctx_id != STARPU_NMAX_SCHED_CTXS &&
      sched_ctx->perf_counters != NULL &&
      _starpu_sched_ctx_allow_hypervisor(sched_ctx_id)) {
    _STARPU_TRACE_HYPERVISOR_BEGIN();
...
{
  return starpu_sched_ctx_get_min_priority(
      _starpu_sched_ctx_get_current_context());
}

int starpu_sched_get_max_priority(void)
{
  return starpu_sched_ctx_get_max_priority(
      _starpu_sched_ctx_get_current_context());
}

int starpu_sched_set_min_priority(int min_prio)
{
  return starpu_sched_ctx_set_min_priority(
      _starpu_sched_ctx_get_current_context(), min_prio);
}

int starpu_sched_set_max_priority(int max_prio)
{
  return starpu_sched_ctx_set_max_priority(
      _starpu_sched_ctx_get_current_context(), max_prio);
}
                                             ...
                                             unsigned priority)
{
  if (nworkers != -1) {
    ...
    for (w = 0; w < nworkers; w++) {
      ...
      _starpu_sched_ctx_list_move(&worker->sched_ctx_list, sched_ctx_id,
                                  priority);
    }
  }
}

void starpu_sched_ctx_set_priority(int *workerids, int nworkers,
                                   unsigned sched_ctx_id, unsigned priority)
{
  if (nworkers != -1) {
    notify_workers_about_changing_ctx_pending(nworkers, workerids);
    _starpu_sched_ctx_lock_write(sched_ctx_id);
...
{
  struct _starpu_worker *worker = _starpu_get_worker_struct(workerid);
  return _starpu_sched_ctx_elt_get_priority(worker->sched_ctx_list,
                                            sched_ctx_id);
}
...
unsigned _starpu_sched_ctx_last_worker_awake(struct _starpu_worker *worker)
{
  ...
  STARPU_ASSERT(_starpu_config.workers[worker->workerid].status &
                STATUS_SLEEPING);
  ...
  while (_starpu_sched_ctx_list_iterator_has_next(&list_it)) {
    struct _starpu_sched_ctx_elt *e =
        _starpu_sched_ctx_list_iterator_get_next(&list_it);
    struct _starpu_sched_ctx *sched_ctx =
        _starpu_get_sched_ctx_struct(e->sched_ctx);

    unsigned last_worker_awake = 1;
    struct starpu_worker_collection *workers = sched_ctx->workers;
    /* workers can be NULL in some corner cases, since we do not lock sched_ctx
     * here */
    if (workers != NULL) {
      struct starpu_sched_ctx_iterator it;
      ...
      while (workers->has_next(workers, &it)) {
        ...
        if (workerid != worker->workerid) {
          ...
          const int cond =
              !(_starpu_config.workers[workerid].status & STATUS_SLEEPING);
...
unsigned starpu_sched_ctx_worker_is_master_for_child_ctx(int workerid,
                                                         unsigned sched_ctx_id)
{
  if (_starpu_get_nsched_ctxs() <= 1) return STARPU_NMAX_SCHED_CTXS;
  ...
  while (_starpu_sched_ctx_list_iterator_has_next(&list_it)) {
    ...
    if (sched_ctx->main_master == workerid &&
        sched_ctx->nesting_sched_ctx == sched_ctx_id)
      return sched_ctx->id;
  }
  return STARPU_NMAX_SCHED_CTXS;
}
...
unsigned starpu_sched_ctx_master_get_context(int masterid)
{
  ...
  while (_starpu_sched_ctx_list_iterator_has_next(&list_it)) {
    ...
    struct _starpu_sched_ctx *sched_ctx =
        _starpu_get_sched_ctx_struct(e->sched_ctx);
    if (sched_ctx->main_master == masterid) return sched_ctx->id;
  }
  return STARPU_NMAX_SCHED_CTXS;
}

struct _starpu_sched_ctx *__starpu_sched_ctx_get_sched_ctx_for_worker_and_job(
    struct _starpu_worker *worker, struct _starpu_job *j)
{
  struct _starpu_sched_ctx_list_iterator list_it;
...
void starpu_sched_ctx_revert_task_counters_ctx_locked(unsigned sched_ctx_id,
                                                      double ready_flops)
{
...
void starpu_sched_ctx_revert_task_counters(unsigned sched_ctx_id,
                                           double ready_flops)
{
  _starpu_decrement_nsubmitted_tasks_of_sched_ctx(sched_ctx_id);
...
void starpu_sched_ctx_move_task_to_ctx_locked(struct starpu_task *task,
                                              unsigned sched_ctx,
                                              unsigned with_repush)
{
  ...
  if (with_repush)
    ...
    _starpu_increment_nready_tasks_of_sched_ctx(j->task->sched_ctx,
                                                j->task->flops, j->task);
}
...
void starpu_sched_ctx_list_task_counters_increment(unsigned sched_ctx_id,
                                                   int workerid)
{
  ...
  /* FIXME: why do we push events only when the worker belongs to more than one
   * ctx? */
...
void starpu_sched_ctx_list_task_counters_decrement(unsigned sched_ctx_id,
                                                   int workerid)
{
  struct _starpu_worker *worker = _starpu_get_worker_struct(workerid);
  if (worker->nsched_ctxs > 1)
...
void starpu_sched_ctx_list_task_counters_reset(unsigned sched_ctx_id,
                                               int workerid)
{
  struct _starpu_worker *worker = _starpu_get_worker_struct(workerid);
  if (worker->nsched_ctxs > 1)
...
void starpu_sched_ctx_list_task_counters_increment_all_ctx_locked(
    struct starpu_task *task, unsigned sched_ctx_id)
{
...
void starpu_sched_ctx_list_task_counters_increment_all(struct starpu_task *task,
                                                       unsigned sched_ctx_id)
{
  /* TODO: add proper, but light-enough locking to sched_ctx counters */
...
void starpu_sched_ctx_list_task_counters_decrement_all_ctx_locked(
    struct starpu_task *task, unsigned sched_ctx_id)
{
  if (_starpu_get_nsched_ctxs() > 1) {
    struct starpu_worker_collection *workers =
...
void starpu_sched_ctx_list_task_counters_decrement_all(struct starpu_task *task,
                                                       unsigned sched_ctx_id)
{
  if (_starpu_get_nsched_ctxs() > 1) {
    struct starpu_worker_collection *workers =
...
void starpu_sched_ctx_list_task_counters_reset_all(struct starpu_task *task,
                                                   unsigned sched_ctx_id)
{
  if (_starpu_get_nsched_ctxs() > 1) {
    _starpu_sched_ctx_lock_write(sched_ctx_id);
                                                        ...
                                                        unsigned all)
{
  ...
  if (temp_master) sched_ctx->main_master = -1;
}
...
void *starpu_sched_ctx_exec_parallel_code(void *(*func)(void *), void *param,
                                          unsigned sched_ctx_id)
{
    ...
    unsigned sched_ctx_id)
{
  ...
  if (sched_ctx->sched_policy) return;
...
void starpu_sched_ctx_get_available_cpuids(unsigned sched_ctx_id, int **cpuids,
                                           int *ncpuids)
{
  int current_worker_id = starpu_worker_get_id();
  struct _starpu_sched_ctx *sched_ctx =
  ...
  while (workers->has_next(workers, &it)) {
    ...
    if (master == current_worker_id || workerid == current_worker_id ||
        current_worker_id == -1) {
...
static void _starpu_sched_ctx_put_new_master(unsigned sched_ctx_id)
{
  ...
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
  unsigned nworkers =
      starpu_sched_ctx_get_workers_list_raw(sched_ctx_id, &workerids);
  unsigned i;
  ...
  STARPU_ASSERT_MSG(
      i < nworkers,
      "StarPU did not find a a CPU worker to be set as the master");
}

struct starpu_perfmodel_arch *_starpu_sched_ctx_get_perf_archtype(
    unsigned sched_ctx_id)
{
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
...
int starpu_sched_ctx_get_worker_rank(unsigned sched_ctx_id)
{
  ...
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
  if (sched_ctx->sched_policy || !sched_ctx->awake_workers) return -1;
  struct starpu_worker_collection *workers = sched_ctx->workers;
  ...
  while (workers->has_next(workers, &it)) {
    int worker = workers->get_next(workers, &it);
    if (worker == curr_workerid) return idx;
    idx++;
  }
...
void (*starpu_sched_ctx_get_sched_policy_callback(unsigned sched_ctx_id))(
    unsigned)
{
  struct _starpu_sched_ctx *sched_ctx =
      _starpu_get_sched_ctx_struct(sched_ctx_id);
...
unsigned starpu_sched_ctx_has_starpu_scheduler(unsigned sched_ctx_id,
...
void _starpu_worker_apply_deferred_ctx_changes(void)
{
  ...
  while (!_starpu_ctx_change_list_empty(l)) {
    ...
    if (chg->nworkers_to_notify) {
      ...
      notify_workers_about_changing_ctx_pending(chg->nworkers_to_notify,
                                                chg->workerids_to_notify);
    } else {
      STARPU_ASSERT(chg->workerids_to_notify == NULL);
      notify_workers_about_changing_ctx_pending(chg->nworkers_to_change,
                                                chg->workerids_to_change);
    }
    _starpu_sched_ctx_lock_write(chg->sched_ctx_id);
    switch (chg->op) {
    case ctx_change_add: {
      add_notified_workers(chg->workerids_to_change, chg->nworkers_to_change,
                           chg->sched_ctx_id);
    } break;
    case ctx_change_remove: {
      remove_notified_workers(chg->workerids_to_change, chg->nworkers_to_change,
                              chg->sched_ctx_id);
      {
        int i;
    ...
    if (chg->nworkers_to_notify) {
      notify_workers_about_changing_ctx_done(chg->nworkers_to_notify,
                                             chg->workerids_to_notify);
    } else {
      notify_workers_about_changing_ctx_done(chg->nworkers_to_change,
                                             chg->workerids_to_change);
    }
...
/* TODO: verify starpu_sched_ctx_create_inside_interval correctness before
 * re-enabling the functions below
 */
#if 0
