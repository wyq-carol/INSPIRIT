...
static void sched_knobs__set(const struct starpu_perf_knob *const knob,
                             void *context,
                             const struct starpu_perf_knob_value *const value)
{
...
static void sched_knobs__get(const struct starpu_perf_knob *const knob,
                             void *context,
                             struct starpu_perf_knob_value *const value)
{
  const char *const sched_policy_name = *(const char **)context;
  (void)sched_policy_name;
  if (knob->id == __s_alpha_knob)
...
{
  {
    ...
    __kg_starpu_dmda__per_scheduler = _starpu_perf_knob_group_register(
        scope, sched_knobs__set, sched_knobs__get);

    /* TODO: priority capping knobs actually work globally for now, the sched
     * policy name is ignored */
    __STARPU_PERF_KNOB_REG("starpu.dmda", __kg_starpu_dmda__per_scheduler,
                           s_alpha_knob, double, "alpha constant multiplier");

    __STARPU_PERF_KNOB_REG("starpu.dmda", __kg_starpu_dmda__per_scheduler,
                           s_beta_knob, double, "beta constant multiplier");

    __STARPU_PERF_KNOB_REG("starpu.dmda", __kg_starpu_dmda__per_scheduler,
                           s_gamma_knob, double, "gamma constant multiplier");

    __STARPU_PERF_KNOB_REG("starpu.dmda", __kg_starpu_dmda__per_scheduler,
                           s_idle_power_knob, double,
                           "idle_power constant multiplier");
  }
}
...
static void _starpu_fifo_task_transfer_started(
    struct starpu_st_fifo_taskq *fifo, struct starpu_task *task,
    int num_priorities)
{
...
/* This is called when a task is actually pushed to the worker (i.e. the
 * transfer finished */
static void _starpu_fifo_task_started(struct starpu_st_fifo_taskq *fifo,
...
static void _starpu_fifo_task_finished(
    struct starpu_st_fifo_taskq *fifo, struct starpu_task *task,
    int num_priorities STARPU_ATTRIBUTE_UNUSED)
{
  if (!isnan(task->predicted))
    /* The execution is over, remove it from pipelined */
    fifo->pipeline_len -= task->predicted;
  fifo->exp_start =
      STARPU_MAX(starpu_timing_now() + fifo->pipeline_len, fifo->exp_start);
  fifo->exp_end = fifo->exp_start + fifo->exp_len;
}
                                    ...
                                    int prio, unsigned sched_ctx_id)
{
  ...
  if (_starpu_get_nsched_ctxs() > 1)
  {
    ...
    if (_starpu_sched_ctx_worker_is_master_for_child_ctx(sched_ctx_id,
                                                         best_workerid, task))
      task = NULL;
  ...
  fifo->exp_start = isnan(fifo->exp_start) ? now + fifo->pipeline_len
  ...
  if (_starpu_get_nsched_ctxs() > 1)
  {
    ...
    if (stream_ctx_id != STARPU_NMAX_SCHED_CTXS)
    {
      ...
      starpu_sched_ctx_revert_task_counters_ctx_locked(sched_ctx_id,
                                                       task->flops);
      _starpu_sched_ctx_unlock_write(sched_ctx_id);
    }
  }
...
static void compute_all_performance_predictions(
    struct starpu_task *task, unsigned nworkers,
    double local_task_length[nworkers][STARPU_MAXIMPLEMENTATIONS],
    double exp_end[nworkers][STARPU_MAXIMPLEMENTATIONS],
    double *max_exp_endp_of_workers, double *min_exp_endp_of_task,
    ...
    unsigned sorted_decision)
{
  ...
  if (sorted_decision && dt->num_priorities != -1)
    task_prio = starpu_st_normalize_prio(task->priority, dt->num_priorities,
                                         sched_ctx_id);
  ...
  while (worker_ctx < nworkers && workers->has_next(workers, &it))
  {
    ...
    struct starpu_perfmodel_arch *perf_arch =
        starpu_worker_get_perf_archtype(workerid, sched_ctx_id);
    ...
    /* Sometimes workers didn't take the tasks as early as we expected */
    double exp_start = isnan(fifo->exp_start)
                           ? now + fifo->pipeline_len
                           : STARPU_MAX(fifo->exp_start, now);
    ...
    for (nimpl = 0; nimpl < STARPU_MAXIMPLEMENTATIONS; nimpl++)
    {
      ...
      if (bundle)
      {
        ...
        if (local_data_penalty)
          local_data_penalty[worker_ctx][nimpl] =
              starpu_task_bundle_expected_data_transfer_time(bundle,
                                                             memory_node);
        if (local_energy)
      ...
      else
      {
        ...
        if (local_data_penalty)
          local_data_penalty[worker_ctx][nimpl] =
              starpu_task_expected_data_transfer_time_for(task, workerid);
        if (local_energy)
          local_energy[worker_ctx][nimpl] = starpu_task_worker_expected_energy(
              task, workerid, sched_ctx_id, nimpl);
      ...
      double ntasks_end =
          fifo_ntasks / starpu_worker_get_relative_speedup(perf_arch);
      ...
      if (local_data_penalty)
        task_starting_time = STARPU_MAX(
            task_starting_time, now + local_data_penalty[worker_ctx][nimpl]);
...
static double _dmda_push_task(struct starpu_task *task, unsigned prio,
                              unsigned sched_ctx_id, unsigned da,
                              unsigned simulate, unsigned sorted_decision)
{
  /* find the queue */
  ...
  /* This is the maximum termination time of already-scheduled tasks over all
   * workers */
  double max_exp_end_of_workers = 0.0;

  double fitness[nworkers_ctx][STARPU_MAXIMPLEMENTATIONS];

  compute_all_performance_predictions(
      task, nworkers_ctx, local_task_length, exp_end, &max_exp_end_of_workers,
      &min_exp_end_of_task, da ? local_data_penalty : NULL,
      ...
      for (nimpl = 0; nimpl < STARPU_MAXIMPLEMENTATIONS; nimpl++)
      {
        ...
        if (da)
          fitness[worker_ctx][nimpl] =
              dt->alpha * __s_alpha__value *
                  (exp_end[worker_ctx][nimpl] - min_exp_end_of_task) +
              dt->beta * __s_beta__value *
                  (local_data_penalty[worker_ctx][nimpl]) +
        ...
        if (da && exp_end[worker_ctx][nimpl] > max_exp_end_of_workers)
        {
          ...
          fitness[worker_ctx][nimpl] +=
              dt->_gamma * __s_gamma__value * dt->idle_power *
              __s_idle_power__value *
              (exp_end[worker_ctx][nimpl] - max_exp_end_of_workers) /
              1000000.0; /* Since gamma is the cost in us of one Joules,
...
static void dmda_add_workers(unsigned sched_ctx_id, int *workerids,
                             unsigned nworkers)
{
  struct _starpu_dmda_data *dt =
      (struct _starpu_dmda_data *)starpu_sched_ctx_get_policy_data(
...
static void dmda_remove_workers(unsigned sched_ctx_id, int *workerids,
                                unsigned nworkers)
{
  struct _starpu_dmda_data *dt =
      (struct _starpu_dmda_data *)starpu_sched_ctx_get_policy_data(
...
static void initialize_dmda_policy(unsigned sched_ctx_id)
{
  ...
  dt->alpha = starpu_get_env_float_default("STARPU_SCHED_ALPHA",
                                           _STARPU_SCHED_ALPHA_DEFAULT);
  dt->beta = starpu_get_env_float_default("STARPU_SCHED_BETA",
                                          _STARPU_SCHED_BETA_DEFAULT);
  /* data->_gamma: cost of one Joule in us. If gamma is set to 10^6, then one
   * Joule cost 1s */
#ifdef STARPU_NON_BLOCKING_DRIVERS
  if (starpu_getenv("STARPU_SCHED_GAMMA"))
    _STARPU_DISP(
        "Warning: STARPU_SCHED_GAMMA was used, but --enable-blocking-drivers "
        "configuration was not set, CPU cores will not actually be sleeping\n");
#endif
  dt->_gamma = starpu_get_env_float_default("STARPU_SCHED_GAMMA",
                                            _STARPU_SCHED_GAMMA_DEFAULT);
  ...
  if (starpu_sched_ctx_min_priority_is_set(sched_ctx_id) != 0 &&
      starpu_sched_ctx_max_priority_is_set(sched_ctx_id) != 0)
    dt->num_priorities = starpu_sched_ctx_get_max_priority(sched_ctx_id) -
                         starpu_sched_ctx_get_min_priority(sched_ctx_id) + 1;
  else
    dt->num_priorities = -1;
...
static void deinitialize_dmda_policy(unsigned sched_ctx_id)
{
...
#ifdef STARPU_VERBOSE
  {
    ...
    _STARPU_DEBUG(
        "%s sched policy (sched_ctx %u): total_task_cnt %ld ready_task_cnt %ld "
        "(%.1f%%), modelled_task_cnt = %ld (%.1f%%)%s\n",
        sched_ctx->sched_policy ? sched_ctx->sched_policy->policy_name
                                : "<none>",
        sched_ctx_id, dt->total_task_cnt, dt->ready_task_cnt,
        (100.0f * dt->ready_task_cnt) / dt->total_task_cnt, modelled_task_cnt,
        (100.0f * modelled_task_cnt) / dt->total_task_cnt,
        modelled_task_cnt == 0 ? " *** Check if performance models are enabled "
                                 "and converging on a per-codelet basis, or "
                                 "use an non-modeling scheduling policy. ***"
                               : "");
  }
#endif
...
static void _dm_push_task_notify(struct starpu_task *task, int workerid,
                                 int perf_workerid, unsigned sched_ctx_id,
                                 int da)
{
  struct _starpu_dmda_data *dt =
      (struct _starpu_dmda_data *)starpu_sched_ctx_get_policy_data(
  ...
  double predicted = starpu_task_worker_expected_length(
      task, perf_workerid, sched_ctx_id, starpu_task_get_implementation(task));
...
static void dm_push_task_notify(struct starpu_task *task, int workerid,
                                int perf_workerid, unsigned sched_ctx_id)
{
